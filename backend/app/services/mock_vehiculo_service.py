...
from app.models.vehiculo import VehiculoCreate, VehiculoUpdate, VehiculoInDB, DatosTecnicos
...
class MockVehiculoService:
    ...
    async def create_vehiculo(self, vehiculo_data: VehiculoCreate) -> VehiculoInDB:
        ...
        vehiculo_dict["fechaRegistro"] = datetime.utcnow()
    ...
    async def get_vehiculos_activos(self) -> List[VehiculoInDB]:
        ...
        return [vehiculo for vehiculo in self.vehiculos.values() if vehiculo.estaActivo]
    ...
    async def get_vehiculos_por_estado(self, estado: str) -> List[VehiculoInDB]:
        ...
        return [vehiculo for vehiculo in self.vehiculos.values() 
                if vehiculo.estado == estado and vehiculo.estaActivo]

    async def get_vehiculos_por_empresa(self, empresa_id: str) -> List[VehiculoInDB]:
        """Obtener vehículos por empresa"""
        return [vehiculo for vehiculo in self.vehiculos.values() 
                if vehiculo.empresaActualId == empresa_id and vehiculo.estaActivo]
    ...
    async def get_vehiculos_con_filtros(self, filtros: Dict) -> List[VehiculoInDB]:
        ...
        if 'empresa_id' in filtros and filtros['empresa_id']:
            vehiculos = [veh for veh in vehiculos if veh.empresaActualId == filtros['empresa_id']]
        
        if 'anio_desde' in filtros and filtros['anio_desde']:
            vehiculos = [veh for veh in vehiculos if veh.anioFabricacion >= filtros['anio_desde']]
        
        if 'anio_hasta' in filtros and filtros['anio_hasta']:
            vehiculos = [veh for veh in vehiculos if veh.anioFabricacion <= filtros['anio_hasta']]
        
        return [veh for veh in vehiculos if veh.estaActivo]

    async def get_estadisticas(self) -> Dict:
        """Obtener estadísticas de vehículos"""
        vehiculos_activos = [veh for veh in self.vehiculos.values() if veh.estaActivo]
    ...
    async def soft_delete_vehiculo(self, vehiculo_id: str) -> bool:
        ...
        if vehiculo_id in self.vehiculos:
            self.vehiculos[vehiculo_id].estaActivo = False
    ...
    async def agregar_ruta_a_vehiculo(self, vehiculo_id: str, ruta_id: str) -> Optional[VehiculoInDB]:
        ...
        if vehiculo_id in self.vehiculos:
            if ruta_id not in self.vehiculos[vehiculo_id].rutasAsignadasIds:
                self.vehiculos[vehiculo_id].rutasAsignadasIds.append(ruta_id)
    ...
    async def remover_ruta_de_vehiculo(self, vehiculo_id: str, ruta_id: str) -> Optional[VehiculoInDB]:
        ...
        if vehiculo_id in self.vehiculos:
            if ruta_id in self.vehiculos[vehiculo_id].rutasAsignadasIds:
                self.vehiculos[vehiculo_id].rutasAsignadasIds.remove(ruta_id)
                return self.vehiculos[vehiculo_id]
        return None
    ...
    async def cambiar_empresa(self, vehiculo_id: str, nueva_empresa_id: str) -> Optional[VehiculoInDB]:
        ...
        if vehiculo_id in self.vehiculos:
            self.vehiculos[vehiculo_id].empresaActualId = nueva_empresa_id
            self.vehiculos[vehiculo_id].fechaActualizacion = datetime.utcnow()
            return self.vehiculos[vehiculo_id]
        return None
