...
export class ResolucionesComponent implements OnInit, OnDestroy {
  ...
  private actualizarURLParams(filtros: any): void {
  ...
  onConfiguracionChange(cambios: any): void {
    console.log('⚙️ Configuración de tabla cambiada:', cambios);
    this.configuracionTabla.set({ ...this.configuracionTabla(), ...cambios });
  }
  ...
  onAccionEjecutada(accion: AccionTabla): void {
    ...
    switch (accion.accion) {
      ...
      case 'ver-rutas-autorizadas':
        if (accion.resolucion) {
          console.log('🛣️ Ver rutas autorizadas:', accion.resolucion.nroResolucion);
        }
        break;
        
      case 'ver-vehiculos-habilitados':
        if (accion.resolucion) {
          console.log('🚗 Ver vehículos habilitados:', accion.resolucion.nroResolucion);
        }
        break;
  ...
  exportarResoluciones(resoluciones?: any[]): void {
  ...
  tieneFiltrosActivos(): boolean {
    const filtros = this.filtrosActuales();
    return Object.keys(filtros).length > 0 && Object.values(filtros).some(v => v !== undefined && v !== null && v !== '');
  }
