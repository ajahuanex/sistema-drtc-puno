      ...
      <mat-card-content>
        @if (codigoEmpresa()) {
          <div class="codigo-display">
            <div class="codigo-numero">{{ numero() }}</div>
            <div class="codigo-letras">{{ letras() }}</div>
          </div>
          
          <div class="tipos-empresa">
            <h4>Tipos de Empresa:</h4>
            <div class="tipos-chips">
              @for (tipo of tiposEmpresa(); track tipo.letra) {
                <mat-chip 
                  [color]="tipo.color" 
                  selected
          ...
          <div class="codigo-details">
            <p><strong>Número Secuencial:</strong> {{ numero() }}</p>
...
export class CodigoEmpresaInfoComponent {
   ...
   */
  codigoEmpresa = input<string>('');

  /**
   * Extrae los primeros 4 dígitos del código de empresa (número secuencial).
   */
  numero = computed(() => {
    const codigo = this.codigoEmpresa();
    return codigo ? codigo.substring(0, 4) : '';
  });
  ...
  letras = computed(() => {
    const codigo = this.codigoEmpresa();
    return codigo ? codigo.substring(4, 7) : '';
  });

  /**
   * Convierte las letras del código en información detallada de tipos de empresa.
   */
  tiposEmpresa = computed(() => {
    const letrasStr = this.letras();
    if (!letrasStr) return [];
